...
private:
  ...
  vector<Float16_t> Electron_ntrk;
  vector<vector<Float16_t>> Electron_d0;
  vector<vector<Float16_t>> Electron_dz;
  vector<vector<Float16_t>> Electron_trkpt;
  vector<vector<Float16_t>> Electron_trketa;
  vector<vector<Float16_t>> Electron_trkphi;
  vector<vector<Float16_t>> Electron_trkchi2overndf;
  ...
  vector<vector<Int_t>> Electron_charge;
...
EGammaOnly_ScoutingNanoAOD::EGammaOnly_ScoutingNanoAOD(const edm::ParameterSet& iConfig): 
  ...
  tree->Branch("Electron_ntrk", &Electron_ntrk);
  ...
  tree->Branch("Electron_trkchi2overndf", &Electron_trkchi2overndf);
...
void EGammaOnly_ScoutingNanoAOD::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  if(eleValid) {
    for (auto &ele : *electronsH) {
      ...
      Electron_ntrk.push_back((electrons_iter->d0()).size());
      ...
      Electron_trkchi2overndf.push_back(electrons_iter->trkchi2overndf());
